...
class Test:
    def __init__(self, robot, obstacles, colision, name, description = "") -> None:
        self.robot = robot
...
def check_end(robot, obstacles):
    ...
    for obstacle in obstacles:
        ...
        if distance <= robot.radius + obstacle.radius:
            obstacle_colision = True  # Los círculos se intersectan o chocan
            break
...
for test in tests_list:
    ...
    anim = animation.FuncAnimation(fig, update, interval=0.1 * 1000)
    anim.running = True
